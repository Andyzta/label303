...
def get_filepath():
    ...
    for i in range(0, len(name)):
        filepath.append(path('dl/' + name[i]+'.csv'))
    return filepath
...
#         return dataframe
def getdata(input_path=path('dl/YC0053.csv')):
        data = pd.read_csv(input_path)
        dict = {'时间': data['时间'],
                'energy': data['energy'],
...
#将前面函数的操作聚合在一起
def dl_datapreprossion(dataframe):
    # dataframe = getdata()
...
def all_dealdata():
    ...
    get_filepath()
    datalist=[]
    dataframe= getdata(filepath[0])
    for i in range(1,len(filepath)):
        data1= getdata(filepath[i])
        # print(dataframe)
        #datalist.append(dataframe)
        dataframe['energy']=dataframe['energy']+data1['energy']
    data=dl_datapreprossion(dataframe)
    ...
    data.to_csv(path('dl/deal'+'电量'+'.csv'), sep=',', index=None, encoding="utf_8_sig")  # 改成覆盖
    print("电量数据处理完成")
