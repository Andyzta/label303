...
def get_filepath():
    ...
    for i in range(0, len(name)):
        filepath.append(path('ll/' + name[i]+'.csv'))
    return filepath
...
#         return dataframe
def getdata(input_path=path('ll/YC456.csv')):
        data = pd.read_csv(input_path)
        dict = {'时间': data['时间'],
                'energy': data['energy'],
...
#将前面函数的操作聚合在一起
def ll_datapreprossion(dataframe):
    # dataframe = getdata()
...
def all_dealdata():
    ...
    for i in range(0, len(filepath)):
        ...
        # print(dataframe)
        data = ll_datapreprossion(dataframe)
        ...
        data.to_csv(path('ll/deal' + name[i]+'.csv'), sep=',', index=None, encoding="utf_8_sig")  # 改成覆盖
        print("冷量数据处理完成")
