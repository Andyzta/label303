...
def path(input_path):
    ...
    # print(dataPath)
    return dataPath
# def getdata(input_path=path('data/20191101-1210.xlsx')):
...
#获取数据
def getdata(input_path=path('data/能量表.csv')):
#中文识别
# def getdata(input_path=path('data/1.csv')):
    data = pd.read_csv(input_path)
    dict = {'时间': data['时间'],
            'ajwldl2.YC456': data['ajwldl2.YC456'],
...
def history_crawlersameY(year,month,day,flag=len(temp)):
    ...
    while j < len(dataS):
        ...
        try:
            ...
            if dataframe['时间'][j][5:7] == str(int(month) + 1):
...
def nl_datapreprossion():
    ...
    #         'YC467': dataframe['YC467'][:len(temp)],
    dict = {'时间': dataframe['时间'],
            'ajwldl2.YC456': dataframe['ajwldl2.YC456'],
    ...
    dataframe = pd.DataFrame(dict)
    print(dataframe)
    dataframe.to_csv(path('data/总能量test.csv'), sep=',', index=None, encoding="utf_8_sig")  # 改成覆盖
if __name__ == '__main__':
    ...
    #
    #
#     # history_crawlersameY(year='2019', month='06', day='01', flag=len(temp))
# if 'ds' in detail[i].keys():
#     if platform.system().lower() == 'windows':
#
...
#         templinux = 5 / 9 * (int(detail[i]['temp']) - 32)
...
# if 'wind' in detail[i].keys():
#     if platform.system().lower() == 'linux':
#         windlinux = str(1.609344 * int(detail[i]['wind']))
#         temp.append(windlinux)
#     elif platform.system().lower() == 'windows':
#         wind.append(detail[i]['wind'])
#
#
