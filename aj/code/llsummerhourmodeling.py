...
def Model_construcion(input_path="../ll/dealYC456.csv", model_path="../ll/xgboostdealYC456.dat"):
	warnings.filterwarnings("ignore")
	data = pd.read_csv(input_path, engine='python', encoding="utf_8_sig")
	data['SDATE'] = pd.to_datetime(data['SDATE'])
	data = data.loc[data['SDATE'].dt.month.isin([6,7,8,9])]
	data.index = range(data.shape[0])
	data=data.loc[(data['energy'] < 300) & (data['energy'] > 50)]
	data = data[326:]
	data.index = range(data.shape[0])
	data.to_csv('../data/datasummer.csv')
	data.index = range(data.shape[0])
	data['SDATE'] = pd.to_datetime(data['SDATE'])
	dataset = data.copy()
	#为后面以日为单位测试，加上日期做准备
	testset = dataset[535:694]
	...
	data = pd.DataFrame({'temphigh': data['temphigh'], 'templow': data['templow'],
						 ...
						 'energy': data['energy']})
	# 分离数据集
	train = pd.concat([data[0:535], data[694:]], axis=0, ignore_index=True)
	test = data[535:694]
	...
	print('预测夏季每小时准确率%5.2f' % compare['准确率'].mean())
	...
	print('预测夏季每天准确率%5.2f' % compare1['准确率'].mean())
	print('预测夏季测试集准确率%5.2f' % (1 - abs(compare1['Y_test'].sum() - compare1['ypreds'].sum()) / compare1['Y_test'].sum()))
...
if __name__ == '__main__':
	Model_construcion(input_path="../ll/dealYC456.csv", model_path="../ll/xgboostdealYC456.dat")